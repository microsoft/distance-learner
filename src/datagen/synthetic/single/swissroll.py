    ...
    def __init__(self, genattrs=None, specattrs=None, N=1000, num_neg=None,\
        ...
        if not isinstance(specattrs, SpecificSwissRollAttrs):
            self._specattrs = SpecificSwissRollAttrs(mu=self._genattrs.mu,\
                 ...
                 g=g, d_g=d_g, t_min=t_min, t_max=t_max, omega=omega, num_turns=num_turns,\
...
if __name__ == '__main__':
    """
    NOTE: say t_min, t_max and omega give you
    a satisfactory swiss roll and you want to
    increase its gap. You would scale omega
    by some factor less than 1. But also scale
    t_min, and t_max by inverse of that factor
    otherwise number of turns will reduce 
    significantly

    Eg. say t_min = 1.5, t_max = 4.5 and omega = np.pi
    was one good configuration. But you want to increase
    the gap by 10 times.

    So you would do omega = np.pi * 0.1. But also you should do
    t_min = 15, t_max = 45, so that number of turns and 
    therefore, the overall shape remains the same.

    Goes without saying that you will have to scale D and max_norm
    suitably if you want to preserve the spread of off-mfld. samples
    as well.
    """
