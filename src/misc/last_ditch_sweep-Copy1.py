...
for eps_iter in np.arange(5e-3, 0.155, 5e-3):
    ...
    all_losses = list()
    all_losses_dl = list()
    all_pred_labels = torch.zeros(nb_iter, num_onmfld)
    all_pred_labels_dl = torch.zeros(nb_iter, num_onmfld)
...
#                                norm=norm, y=true_classes)
        adv_eg, _, pred_labels = pgd_l2_mfld_clf_attack(stdclf_model, inputs, true_classes, eps_iter=eps_iter, nb_iter=nb_iter, norm=norm, verbose=True)
...
#         deltas_ma_dl = pgd_dist(distlearn_model, inputs, true_distances, eps=eps, eps_iter=eps_iter, nb_iter=nb_iter, norm=norm) 
        deltas_ma_dl, _, pred_labels_dl = pgd_l2_mfld_dist_attack(distlearn_model, inputs, true_distances, eps_iter=eps_iter, nb_iter=nb_iter, norm=norm, verbose=True)
        all_pred_labels[:, start:end] = pred_labels
        all_pred_labels_dl[:, start:end] = pred_labels_dl
    ...
    torch.save(all_pred_labels, "pred_labels_for_{}_onmfld_1kiters.pth".format(eps_iter))
    torch.save(all_pred_labels_dl, "pred_labels_dl_for_{}_onmfld_1kiters.pth".format(eps_iter))
    ...
    with open("sweep_logs2onmfld_v3.json", "w") as f:
